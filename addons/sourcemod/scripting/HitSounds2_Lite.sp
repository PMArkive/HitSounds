...
// Hitsound File Path (Hardcoded for now... Will be changed to CVar)
#define SND_PATH_HITSOUND "hitmarker/hitmarker.mp3"

bool g_bLate = false;

public Plugin myinfo = 
{
	name        = "HitSounds v2 Lite",
...
public void Hook_EntityOnDamage(const char[] output, int caller, int activator, float delay)
{
	...
	if (g_bHearSound[activator] && g_fHitVolumeSound[activator] != 0.0)
		EmitSoundToClient(activator, SND_PATH_HITSOUND, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, g_fHitVolumeSound[activator]);
}
...
public void Hook_EventOnDamage(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		EmitSoundToClient(iAttacker, SND_PATH_HITSOUND, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, g_fHitVolumeSound[iAttacker]);
	}
}
...
stock void PrecacheSounds()
{
	...
	// Hitmarker Sound
	PrecacheSound(SND_PATH_HITSOUND, true);
	Format(sBuffer, sizeof(sBuffer), "sound/%s", SND_PATH_HITSOUND);
